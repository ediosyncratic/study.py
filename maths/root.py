...
class Search:
    ...
    def __init__(self, func, guess, goal=abs, stride=1):
        ...
        # Determine whether to use evaluate or not:
        try: first = guess.evaluate(func)
        except AttributeError:
            first = func(guess)
            self.__func = func
        else: self.__func = lambda x, f=func: x.evaluate(f)

        # Initialise cache, best
        self.__cache = { guess: first }
        self.__best = (guess, goal(first))
    ...
    def logrange(scale=1, count=10, step=-.5):
        row = []
        while count > 0:
            row.append(scale)
    ...
    def Raphson(self, val=None, step=None):
        if val is None: val = self.best
        move = - self.func(val) / self.gradient(val, step)
        if move: self.stride = move
        ans = val + move
        return ans, self.goal(self.func(ans))

    def chord(self, val=None, step=None):
        if val is None: val = self.best
    ...
    def rummage(self, tries=42, threshold=-1):
        ...
        while tries > 0:
            ...
            if best < self.score:
                if self.score < threshold: return self.__best
                best = self.score

_rcs_log = """
 $Log: root.py,v $
