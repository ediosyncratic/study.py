...
class Planckoid (Object):
    ...
    temperature, acceleration - each with its conventional meaning.\n"""
...
charge, meanwhile, is of order eight positrons-worth - so manifestly reducible
- and the universe clearly *does* quantise charge (albeit possibly in units of
a third that on the positron) so even when a quantity does have a `minimal
scale' this needn't be the relevant Planck unit.
...
to use the Einstein-Maxwell charge-to-mass ratio (Cosmos.qperm, above) in
