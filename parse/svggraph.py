...
$Id: svggraph.py,v 1.1 2009-10-16 06:19:23 eddy Exp $
"""
...
class SVGgraph (Cached):
    ...
    Constructor takes the path of the file containing the SVG: if this does not
    exist, a template is used and the new object's .pristine attribute is set to
    True (it is, otherwise, False).  In this case, you must call its .label()
    and .add_curves() to configure it, before trying to add any data to it; you
    should also add some data to the curves.  The file-name specified to the
    constructor is remembered for later use when .save() is called.\n"""
    ...
    import os
    __template = os.path.join(os.path.split(__file__)[0], 'graph-template.svg')
    pristine = False
